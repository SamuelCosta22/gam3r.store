...
export default function CabecalhoCheckout(props: CabecalhoCheckoutProps) {
    ...
    function bkSelecionado(passo: string) {
        return props.passo === passo ? 'bg-pink-500 text-white' : 'bg-zinc-400 text-black'
    }
    ...
    function renderizarItem(
        ...
        return (
            <Link href={caminho} className={`flex items-center gap-2 cursor-pointer ${corSelecionada(passo)}`}>
                <span className={`flex justify-center items-center text-xs font-bold w-5 h-5 rounded-full ${bkSelecionado(passo)}`}>
                    {indice}
                </span>
                <span>{titulo}</span>
            </Link>
